...
## 5.2.8 Выбор роли порта в разныз случаях
На картинке ниже, BPDU от S3, заставил S2 признать порт F0/2 ALTN и заблокировать его

![](./pictures/39.jpg)

На следующей картинке, происходит сравнение BID. Видно что BID S1< BID S2, значит блокироваться будет какой-то порт на стороне S2:
- BID одинаковое для обоих линкОв
- надо сверять стоимость портов (по-умолчанию она 128 и не менялась на портах)
